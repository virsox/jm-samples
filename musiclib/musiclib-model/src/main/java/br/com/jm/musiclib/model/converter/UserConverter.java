...
public class UserConverter implements Converter<User> {
  ...
  /**
   * Construtor.
   * @param playlistConverter Conversor de objetos Playlist. Injetado pelo CDI.
   */
  @Inject
  ...
  public DBObject toDBObject(User user) {
    DBObject doc = BasicDBObjectBuilder.start().add("name", user.getName())
        .add("login", user.getLogin()).add("password", user.getPassword())
        .get();
    ...
    for (Map.Entry<String, Integer> entry : user.getExecutions().entrySet()) {
      DBObject execucao = BasicDBObjectBuilder.start()
          ...
          .add("quantity", entry.getValue()).get();
  ...
  public User toObject(DBObject doc) {
    ...
    for (DBObject executionDoc : executionsDocs) {
      executions.put(((ObjectId) executionDoc.get("music")).toString(),
          (Integer) executionDoc.get("quantity"));
    }

    User user = new User(((ObjectId) doc.get("_id")).toString(),
        (String) doc.get("name"), (String) doc.get("login"),
        (String) doc.get("password"), playlists, executions);
