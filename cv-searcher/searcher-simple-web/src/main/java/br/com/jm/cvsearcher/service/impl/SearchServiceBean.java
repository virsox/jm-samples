...
public class SearchServiceBean extends AbstractLuceneBean implements
   ...
   */
	...
	public List<SearchResult> findCVByContent(String content)
		...
		Query q = null;
		try {
			q = new QueryParser(Version.LUCENE_35, Constants.FIELD_CONTENT,
	...
	private List<SearchResult> executeQuery(Query q, String field)
		...
		try {
			...
			// Hightliter para poder marcar os termos encontrados
			...
			for (int i = 0; i < hits.length; ++i) {
				...
				// Destacar as palavras encontradas
				TokenStream tokenStream = analyzer.tokenStream(field,
						new StringReader(cv.getContent()));

				// Alterar o conteúdo do CV para o conteúdo com os termos em
				// destaque
				cv.setContent(highlighter.getBestFragment(tokenStream,
						cv.getContent()));
			...
			throw new CurriculumException(
					"Erro ao destacar os termos no resultado.", e);
		}
